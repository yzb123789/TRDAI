...
class History(Callback):
    ...
    def on_train_begin(self, logs=None):
        """Initialize history and other parameters at the start of training."""
        self.history = {"loss": []}
        self.history.update({"lr": []})
        self.history.update({name: [] for name in self.trainer._metrics_names})
        self.start_time = logs["start_time"]
        self.epoch_loss = 0.0  # Ensure epoch_loss is initialized

    def on_epoch_begin(self, epoch, logs=None):
        """Reset values at the start of each epoch."""
        self.epoch_metrics = {"loss": 0.0}
        self.samples_seen = 0.0
        self.epoch_loss = 0.0  # Reset epoch_loss at the beginning of each epoch

    def on_epoch_end(self, epoch, logs=None):
        """At the end of each epoch, calculate average loss and update history."""
        # Calculate average loss for the epoch
        if self.samples_seen > 0:
            avg_loss = self.epoch_loss / self.samples_seen
        else:
            avg_loss = 0.0
        
        self.epoch_metrics["loss"] = avg_loss  # Store the average loss for the epoch

        # Update the history with the epoch metrics
        ...
        # Calculate total time and format the message
        ...
        # Save to tensorboard if trainer has a log
        if self.trainer.log:
            self.trainer.log.save_tensorboard(self.epoch_metrics, epoch)

    def on_batch_end(self, batch, logs=None):
        """At the end of each batch, update the epoch loss."""
        batch_size = logs["batch_size"]
        batch_loss = logs["loss"]
        
        # Update epoch_loss with weighted average
        self.epoch_loss = (
            self.samples_seen * self.epoch_loss + batch_size * batch_loss
        ) / (self.samples_seen + batch_size)
        
        # Update the total number of samples seen so far in the epoch
        self.samples_seen += batch_size

    def __getitem__(self, name):
        """Allow indexing into history."""
    ...
    def __str__(self):
        """Return a string version of the history."""
        return str(self.history)
