...
class MetricContainer:
    ...
    def __call__(self, y_true, y_pred):
        ...
        for metric in self.metrics:
            ...
            # Optionally store the result with maximize flag
            if metric._maximize:
                logs[f"{self.prefix}{metric._name}_maximize"] = res
...
class Metric:
    ...
    @classmethod
    def get_metrics_by_names(cls, names, maximize_dict=None):
        ...
        names : list of str
            List of metric names.
        maximize_dict : dict, optional
            Dictionary where the key is the metric name and the value is whether 
            that metric should be maximized (True) or minimized (False). 
            Default is None, which means use default values (False).
        ...
        if maximize_dict is None:
            maximize_dict = {}

        for name in names:
            assert name in available_names, f"{name} is not available, choose from {available_names}"
            idx = available_names.index(name)
            metric = available_metrics[idx]()

            # Determine whether this metric should be maximized
            maximize = maximize_dict.get(name, False)  # Default to False if not specified
            metric._maximize = maximize
