...
class History(Callback):
    ...
    def on_train_begin(self, logs=None):
        # Initialize history and add a placeholder for each metric
        self.history = {"loss": []}
        self.history.update({"lr": []})
        self.history.update({name: [] for name in self.trainer._metrics_names})  # Ensure dynamic metric tracking
    ...
    def on_epoch_end(self, epoch, logs=None):
        # Update history with the epoch metrics
        self.epoch_metrics["loss"] = self.epoch_loss
        for metric_name, metric_value in self.epoch_metrics.items():
            if metric_name not in self.history:
                self.history[metric_name] = []  # Ensure metric is added if it's new
            self.history[metric_name].append(metric_value)

        # Dynamically add new metrics (such as 'auroc') from the logs
        for metric_name, metric_value in logs.items():
            if metric_name not in self.history:
                self.history[metric_name] = []  # Ensure new metrics are tracked
            self.history[metric_name].append(metric_value)
