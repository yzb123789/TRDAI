...
from model.WCADANet import WCADANet
import torch.nn as nn
from sklearn.base import BaseEstimator
from sklearn.utils import check_array

@dataclass
class WCADANsModel(BaseEstimator):
    ...
    def predict(self, X):
        ...
        for batch_nb, data in enumerate(dataloader):
            ...
            with torch.no_grad():
                output = self.network(data)  # Assuming this is the raw score (logits)
                # print(output)
                # predictions = output.cpu().detach().numpy() 
                predictions = torch.sigmoid(output).cpu().detach().numpy()  # Apply sigmoid for probabilities
            results.append(predictions)
        res = np.vstack(results)
        # print(res)
        return res  # Return the probabilities (not hard labels)



    def save_check(self):
            """
            Complete logic for saving the model, ensuring all parameters are written into the dictionary
            """
            # Construct the complete saving dictionary
            save_dict = {
                'epoch': self.epoch,
                'model': self.network,                 # Save model object (optional)
                'optimizer': self._optimizer.state_dict(),
                'state_dict': self.network.state_dict(), # Explicitly save the weights dictionary
            ...
            # If a record is broken, save the Best
    ...
    def _predict_epoch(self, name, loader):
        ...
        for batch_idx, (X, y) in enumerate(loader):
            scores = self._predict_batch(X)  # Get predicted scores (probabilities)
            # print(scores)
        ...
        # Calculate AUROC if it's a classification task
        if self._task == 'classification':
            # print(y_true)
            # print(scores)
            ...
            print(f"Validation AUROC: {valid_auroc:.5f}")

        # Update metrics for regression (optional) or any other metrics you might have
        if self._task == 'regression':
            for k, v in metrics_logs.items():
                metrics_logs[k] = v * self.std ** 2  # For regression, you might scale back the metrics
    ...
    def stack_batches(self, list_y_true, list_y_score):
        """
        Stack the batches of predictions and true values.
        Parameters
        ----------
        list_y_true : list of np.array
            List of true labels.
        list_y_score : list of np.array
            List of predicted scores (probabilities).
        Returns
        -------
        y_true : np.array
            Flattened array of true labels.
        y_score : np.array
            Flattened array of predicted scores.
        """
        y_true = np.concatenate(list_y_true, axis=0)
        y_score = np.concatenate(list_y_score, axis=0)
    ...
    def _set_network(self):
        ...
        print("===> Building WCA-DANet model ...")
        params = {
            'layer_num': self.layer,
            'base_outdim': self.base_outdim,
            'k': self.k,
        ...
        self.network = WCADANet(self.input_dim, self.output_dim, **params)
