...
def save_model(model):
    model_path = r'../data/BiSLANet_data/output/model/'
    ...
    def test(self, dataloader="test"):
            ...
            with torch.no_grad():
                ...
                for i, (v_d, v_p, v_s, labels) in enumerate(data_loader):
                    ...
                    # Use the current model for the validation set and the saved best model for the test set
                    if dataloader == "val":
            ...
            if dataloader == "test":
                ...
                # Find the threshold where Precision = 0.900
                target_precision = 0.900
                # Exclude the last value of precisions (which is typically 1 and has no corresponding threshold)
                idx = np.argmin(np.abs(prec[:-1] - target_precision))
                thred_optim = thresholds[idx]

                # Binarize the output based on the threshold corresponding to 0.9 Precision
                ...
                #Recalculate F1 and Precision at this threshold
                ...
                data.to_csv('../data/BiSLANet_data/output/visualization.csv', index=False)
